...
from app.models.cpp_topic_catalog import CPP_TOPICS

SECTION_ORDER = ["information", "examples", "programming", "fill-blanks", "test"]


def get_or_create_cpp_progress(db: Session, student_id: str, topic_id: int) -> CPPStudentProgress:
    """Retrieve existing C++ progress or initialize standard progress state based on prerequisites."""
    progress = (
        db.query(CPPStudentProgress)
        .filter(CPPStudentProgress.student_id == str(student_id), CPPStudentProgress.topic_id == topic_id)
        .first()
    )
    ...
    if topic_id == 1:
        ...
        prev_progress = (
            db.query(CPPStudentProgress)
            .filter(CPPStudentProgress.student_id == str(student_id), CPPStudentProgress.topic_id == topic_id - 1)
            .first()
        )
...
def validate_cpp_topic_unlocked(progress: CPPStudentProgress):
...
def validate_cpp_section_access(progress: CPPStudentProgress, target_section: str):
    """
    Enforce backend C++ learning sequence:
    Information -> Examples -> Programming -> Fill in Blanks -> SkillExa Test
    """
    validate_cpp_topic_unlocked(progress)
...
def complete_cpp_information_section(db: Session, progress: CPPStudentProgress) -> CPPStudentProgress:
    validate_cpp_topic_unlocked(progress)
...
def complete_cpp_examples_section(db: Session, progress: CPPStudentProgress) -> CPPStudentProgress:
    validate_cpp_section_access(progress, "examples")
...
def complete_cpp_programming_section(db: Session, progress: CPPStudentProgress) -> CPPStudentProgress:
    validate_cpp_section_access(progress, "programming")
...
def complete_cpp_fill_blanks_section(db: Session, progress: CPPStudentProgress) -> CPPStudentProgress:
    validate_cpp_section_access(progress, "fill-blanks")
...
def submit_cpp_skill_exa_test(
    db: Session,
    progress: CPPStudentProgress,
    user_answers: dict[str, str] | list[str] | None,
) -> tuple[CPPStudentProgress, float, dict | None]:
    """
    ...
    """
    validate_cpp_section_access(progress, "test")

    topic = CPP_TOPICS.get(progress.topic_id)
    if not topic:
    ...
    if next_topic_id in CPP_TOPICS:
        next_progress = get_or_create_cpp_progress(db, progress.student_id, next_topic_id)
        ...
        next_topic_raw = CPP_TOPICS[next_topic_id]
        next_topic_info = {
            "id": next_topic_id,
