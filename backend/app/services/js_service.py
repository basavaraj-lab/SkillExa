"""Service layer for JavaScript learning flow, sequence enforcement, and database progress management."""
from fastapi import HTTPException
from sqlalchemy.orm import Session
from app.models.js_progress import JSStudentProgress
from app.models.js_topic_catalog import JS_TOPICS

SECTION_ORDER = ["information", "examples", "programming", "fill-blanks", "test"]


def get_or_create_js_progress(db: Session, student_id: str, topic_id: int) -> JSStudentProgress:
    """Retrieve existing JavaScript progress or initialize standard progress state based on prerequisites."""
    progress = (
        db.query(JSStudentProgress)
        .filter(JSStudentProgress.student_id == str(student_id), JSStudentProgress.topic_id == topic_id)
        .first()
    )
    ...
    if topic_id == 1:
        ...
        prev_progress = (
            db.query(JSStudentProgress)
            .filter(JSStudentProgress.student_id == str(student_id), JSStudentProgress.topic_id == topic_id - 1)
            .first()
        )
...
def validate_js_topic_unlocked(progress: JSStudentProgress):
    """Ensure the JavaScript topic is not locked."""
    if progress.status == "LOCKED":
        raise HTTPException(
            status_code=403,
            detail="This JavaScript topic is locked. Complete the previous topic first!",
        )


def validate_js_section_access(progress: JSStudentProgress, target_section: str):
    """
    Enforce backend JavaScript learning sequence:
    Information -> Examples -> Programming -> Fill in Blanks -> SkillExa Test
    """
    validate_js_topic_unlocked(progress)
...
def complete_js_information_section(db: Session, progress: JSStudentProgress) -> JSStudentProgress:
    validate_js_topic_unlocked(progress)
...
def complete_js_examples_section(db: Session, progress: JSStudentProgress) -> JSStudentProgress:
    validate_js_section_access(progress, "examples")
...
def complete_js_programming_section(db: Session, progress: JSStudentProgress) -> JSStudentProgress:
    validate_js_section_access(progress, "programming")
...
def complete_js_fill_blanks_section(db: Session, progress: JSStudentProgress) -> JSStudentProgress:
    validate_js_section_access(progress, "fill-blanks")
...
def submit_js_skill_exa_test(
    db: Session,
    progress: JSStudentProgress,
    user_answers: dict[str, str] | list[str] | None,
) -> tuple[JSStudentProgress, float, dict | None]:
    """
    Validate JS SkillExa Test completion, compute test score, mark topic COMPLETED,
    unlock next topic in database, and return next topic info.
    """
    validate_js_section_access(progress, "test")

    topic = JS_TOPICS.get(progress.topic_id)
    if not topic:
        raise HTTPException(status_code=404, detail="JavaScript Topic content not found.")
    ...
    if next_topic_id in JS_TOPICS:
        next_progress = get_or_create_js_progress(db, progress.student_id, next_topic_id)
        ...
        next_topic_raw = JS_TOPICS[next_topic_id]
        next_topic_info = {
            "id": next_topic_id,
